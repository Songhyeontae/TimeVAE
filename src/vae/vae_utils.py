...
from vae.vae_dense_model import VAE_Dense
from vae.vae_conv_model import VAE_Conv
from vae.timevae import TimeVAE
from vae.timegdvae import TimeGDVAE
...
def instantiate_vae_model(
...
) -> Union[VAE_Dense, VAE_Conv, TimeVAE, TimeGDVAE]:
    """
    ...
    Args:
        vae_type (str): The type of VAE model to instantiate.
                        One of ('vae_dense', 'vae_conv', 'timeVAE', 'timeGDVAE').
        sequence_length (int): The sequence length.
        feature_dim (int): The feature dimension.
        batch_size (int): Batch size for training.

    Returns:
        Union[VAE_Dense, VAE_Conv, TimeVAE, TimeGDVAE]: The instantiated VAE model.
    ...
    if vae_type == "vae_dense":
        ...
        raise ValueError(
            ...
            "Please choose from vae_dense, vae_conv, timeVAE, TimeGDVAE."
        )
...
def train_vae(vae, train_data, max_epochs, verbose=0):
    ...
    Args:
        vae (Union[VAE_Dense, VAE_Conv, TimeVAE, TimeGDVAE]): The VAE model to train.
...
def save_vae_model(vae, dir_path: str) -> None:
    ...
    Args:
        vae (Union[VAE_Dense, VAE_Conv, TimeVAE, TimeGDVAE]): The VAE model to save.
...
def load_vae_model(vae_type: str, dir_path: str) -> Union[VAE_Dense, VAE_Conv, TimeVAE]:
    ...
    Returns:
        Union[VAE_Dense, VAE_Conv, TimeVAE, TimeGDVAE]: The loaded VAE model.
    """
    if vae_type == "vae_dense":
...
def get_posterior_samples(vae, data):
    ...
    Args:
        vae (Union[VAE_Dense, VAE_Conv, TimeVAE, TimeGDVAE]): The trained VAE model.
